# GPU Accelerated Spread Pipeline

## Scientific Purpose

This pipeline runs the `firefront` wildfire workflow through public Sprtz wrappers and renders publication-ready diagnostics from the NetCDF product.
...
Products are written under `data/09_gpu_accelerated_spread/` by default. Set `SPRTZ_DATA_ROOT` to change the data root or `SPRTZ_OUTPUT_DIR` to choose the exact output directory. The script sets `MPLCONFIGDIR` and `XDG_CACHE_HOME` under the output directory by default for clean headless rendering.
