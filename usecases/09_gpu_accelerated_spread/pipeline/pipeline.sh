...
log_step "4. GPU Accelerated Spread model execution"
python3 "${SCRIPTS_DIR}/sprtzfire.py" --config "${REPO_ROOT}/examples/wildfire_minimal.json" --output-dir "${OUT_DIR}/model" --interchange netcdf --gpu-backend auto

log_step "5. Publication-ready 2-D meteorological context map"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/plotter.py" "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_map.png" --variable wind_speed --title "GPU Accelerated Spread" --dpi 600

log_step "6. Publication-ready meteorological context profile"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/plotter.py" profile "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_profile.png" --variable wind_speed --x 0 --y 0 --title "GPU Accelerated Spread Profile" --config "${REPO_ROOT}/examples/wildfire_minimal.json" --dpi 600

log_step "7. Publication-ready 3-D meteorological context surface"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/plotter.py" render3d "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_3d.png" --variable wind_speed --title "GPU Accelerated Spread 3-D" --config "${REPO_ROOT}/examples/wildfire_minimal.json" --dpi 600 --mode surface --view northeast --vertical-exaggeration 3
