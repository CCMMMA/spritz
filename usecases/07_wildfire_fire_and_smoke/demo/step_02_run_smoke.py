...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the coupled wildfire smoke stage")
    add_plot_argument(parser)
    args = parser.parse_args(argv)
    root = Path(__file__).resolve().parents[2]
    out = root / "output_fire_smoke"
    workflow = run_workflow(root / "examples" / "wildfire_minimal.json", out, backend="fire+puff", interchange="netcdf")
    if args.plot:
