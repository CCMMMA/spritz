# Acerra Waste-to-Energy Pipeline

## Scientific Purpose

This pipeline implements Waste-to-energy stack emission scenario with deterministic meteorology. It synthesizes a public Sprtz configuration, runs the model through command-line wrappers, and renders publication-ready figures.
...
Products are written under `data/06_acerra_waste_to_energy/` by default. Set `SPRTZ_DATA_ROOT` to change the data root or `SPRTZ_OUTPUT_DIR` to choose the exact output directory. The script sets `MPLCONFIGDIR` and `XDG_CACHE_HOME` under the output directory by default for clean headless rendering.
...
- `DX` defaults to `100`.
- `DY` defaults to `100`.
- `EMISSION_RATE_G_S` defaults to `18.0`.
- `WIND_SPEED_M_S` defaults to `3.5`.
- `SOURCE_X_M` defaults to `1200.0`.
- `SOURCE_Y_M` defaults to `1200.0`.

All parameters may be overridden as shell environment variables.

## Expected Products

- `acerra_waste_to_energy_config.json`
- `model/meteo.nc`
