...
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/render.py" "${CONC_PATH}" --output "${FIGURE_DIR}/concentration_map.png" --variable concentration_field --title "Acerra WTE concentration" --dpi 600 --level-index 1 --vector-density 18

log_step "6. Publication-ready vertical concentration profile"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/profiler.py" "${CONC_PATH}" --output "${FIGURE_DIR}/concentration_profile.png" --variable concentration_field --x "${SOURCE_X_M}" --y "${SOURCE_Y_M}" --title "Acerra WTE concentration Profile" --config "${CONFIG_PATH}" --dpi 600

log_step "7. Publication-ready 3-D concentration surface"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/render3d.py" "${CONC_PATH}" --output "${FIGURE_DIR}/concentration_3d.png" --variable concentration_field --title "Acerra WTE concentration 3-D" --config "${CONFIG_PATH}" --dpi 600 --mode surface --view northeast --vertical-exaggeration 3
