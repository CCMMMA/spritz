...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the FIRMS satellite ignition demo")
    add_plot_argument(parser)
    args = parser.parse_args(argv)
    root = Path(__file__).resolve().parents[2]
    out = root / "output_firms"
    workflow = run_workflow(root / "examples" / "wildfire_minimal.json", out, backend="firms+fire", interchange="netcdf")
    if args.plot:
