# FIRMS Satellite Ignition Pipeline

## Scientific Purpose

This pipeline runs a credential-free FIRMS-compatible offline ignition surrogate through public Sprtz wrappers and renders publication-ready diagnostics from the NetCDF product.
...
Products are written under `data/08_firms_satellite_ignition/` by default. Set `SPRTZ_DATA_ROOT` to change the data root or `SPRTZ_OUTPUT_DIR` to choose the exact output directory. The script sets `MPLCONFIGDIR` and `XDG_CACHE_HOME` under the output directory by default for clean headless rendering.
...
- `model/meteo.nc`
- `model/firefront.nc` and `meteo_context.nc`
- `model/post.json`
...
Runs `scripts/sprtzfire.py` on the local wildfire example as an offline surrogate. Operational FIRMS ingestion must be added as an explicit upstream credentialed acquisition step.
...
- FIRMS credentials are not embedded or logged; this script-only pipeline uses an offline surrogate for repeatable smoke execution.
