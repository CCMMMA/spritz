...
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/render.py" "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_map.png" --variable wind_speed --title "FIRMS Satellite Ignition" --dpi 600

log_step "6. Publication-ready meteorological context profile"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/profiler.py" "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_profile.png" --variable wind_speed --x 0 --y 0 --title "FIRMS Satellite Ignition Profile" --config "${REPO_ROOT}/examples/wildfire_minimal.json" --dpi 600

log_step "7. Publication-ready 3-D meteorological context surface"
MPLBACKEND=Agg python3 "${REPO_ROOT}/tools/render3d.py" "${CONC_PATH}" --output "${FIGURE_DIR}/meteo_context_3d.png" --variable wind_speed --title "FIRMS Satellite Ignition 3-D" --config "${REPO_ROOT}/examples/wildfire_minimal.json" --dpi 600 --mode surface --view northeast --vertical-exaggeration 3
