...
from pypuff.models import calmet, calpuff, particles
from pypuff.parallel import get_mpi_context, partition_indices
...
def test_gaussian_parallel_serial_equivalence(tmp_path):
    ...
    serial = calpuff.run(cfg, meteo_path, tmp_path / "serial.csv", "csv", parallel="serial")
    auto = calpuff.run(cfg, meteo_path, tmp_path / "auto.csv", "csv", parallel="auto")
    assert auto == serial
